...
namespace Micropolis
{
    ...
    public sealed partial class MicropolisDrawingArea : IMapListener
    {
        ...
        private void LayoutRoot_PointerMoved(object sender, PointerRoutedEventArgs e)
        {
            ...
            {
                PointerPoint ptrPt = e.GetCurrentPoint(LayoutRoot);

                if (ptrPt.Properties.IsRightButtonPressed)
                {
                    ContinueDrag((int) e.GetCurrentPoint(LayoutRoot).Position.X,
                        (int) e.GetCurrentPoint(LayoutRoot).Position.Y);
                }

                if (!ptrPt.Properties.IsLeftButtonPressed && !ptrPt.Properties.IsRightButtonPressed)
                {
                    PositionToolCursor(e.GetCurrentPoint(this).Position.X, e.GetCurrentPoint(this).Position.Y);
                }
                else if (dragging)
                {
                    PositionToolCursor(Math.Min(dragX, e.GetCurrentPoint(this).Position.X),
                        Math.Min(dragY, e.GetCurrentPoint(this).Position.Y));
                }
            }
        }
        ...
        public void PositionToolCursor(double mouseX, double mouseY)
        {
            ...
            if (!(ImageCursor.RenderTransform is MatrixTransform))
            {
                exTr = (TranslateTransform) ImageCursor.RenderTransform;
                newX = exTr.X;
                newY = exTr.Y;
            }
            ...
            if (toolCursor != null)
            {
                ...
                {
                    ImageCursor.Visibility = Visibility.Collapsed;
                }
            }
            else
            {
            ...
            if (positionNeedsToBeUpdated)
            {
                ...
                ImageCursor.RenderTransform = transTrans;
            }
        }
        ...
        private void LayoutRoot_PointerReleased(object sender, PointerRoutedEventArgs e)
        {
            ...
            if (ptr.PointerDeviceType == PointerDeviceType.Mouse)
            {
                ...
                EndDrag((int) e.GetCurrentPoint(LayoutRoot).Position.X,
                    (int) e.GetCurrentPoint(LayoutRoot).Position.Y);
            }
        }
        ...
        private void LayoutRoot_PointerPressed(object sender, PointerRoutedEventArgs e)
        {
            ...
            if (ptr.PointerDeviceType == PointerDeviceType.Mouse)
            {
                ...
                {
                    StartDrag((int) e.GetCurrentPoint(LayoutRoot).Position.X,
                        (int) e.GetCurrentPoint(LayoutRoot).Position.Y);
                }
            }
        ...
        private async Task DrawSprite(WriteableBitmap gr, Sprite sprite)
        {
            ...
            else
            {
                ...
                await
                    gr.DrawString((sprite.Frame - 1).ToString(), (int) p.X, (int) p.Y, Colors.White, TextBlockToRender,
                        SPToRender);
            ...
            int paintHeight = int.MaxValue/2)
        {
            ...
            if (imageNotCreated || imageHasDifferentsize)
            {
                ...
                Dispatcher.RunAsync(CoreDispatcherPriority.Normal, () =>
                {
                    ...
                    ImageCursor.Height = height*TILE_HEIGHT;
                    ImageCursor.Width = width*TILE_WIDTH;
                    ImageCursor.Stretch = Stretch.None;
                });
            ...
            if (imageNotCreated || imageHasDifferentsize)
            {
                ImageOutput.Source = Image;
                ImageCursor.Source = cursor;
            }
        }
                            ...
                            for (int col = 0; col < toolPreview.GetWidth(); col++)
                            {
                                ...
                                if (needsUpdate)
                                {
                                    ...
                                    using (img.GetBitmapContext(ReadWriteMode.ReadOnly))
                                    {
                                        int shiftedX = 0;
                                        int shiftedY = 0;
                                        if (ImageCursor.RenderTransform is TranslateTransform)
                                        {
                                            shiftedX = x
                                                       - (int)
                                                           Math.Ceiling(
                                                               (ImageCursor.RenderTransform as TranslateTransform).X
                                                               /TILE_WIDTH);
                                            shiftedY = y
                                                       - (int)
                                                           Math.Ceiling(
                                                               (ImageCursor.RenderTransform as TranslateTransform).Y
                                                               /TILE_HEIGHT);
                                        }
                                        cursor.DrawInto(img,
